...
@interface UQUIKFormField ()<UQUIKTextFieldEditDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        UQUIKTextField *textField = [UQUIKTextField new];
        ...
        [UQUIKAppearance styleLabelBoldPrimary:self.formLabel];
...
- (void)drawRect:(CGRect)rect {
    // Draw borders
    [[UQUIKAppearance sharedInstance].lineColor setFill];
...
- (void)updateAppearance {
    ...
    if (!self.displayAsValid){
        textColor = [UQUIKAppearance sharedInstance].errorForegroundColor;
        ...
        textColor = [UQUIKAppearance sharedInstance].primaryTextColor;
        if (currentAccessibilityLabel != nil) {
    ...
    [mutableText addAttributes:@{NSForegroundColorAttributeName: textColor, NSFontAttributeName:[UIFont fontWithName:[UQUIKAppearance sharedInstance].fontFamily size:[UIFont labelFontSize]]} range:NSMakeRange(0, mutableText.length)];
...
#pragma mark - UQUITextFieldEditDelegate methods

- (void)textFieldWillDeleteBackward:(__unused UQUIKFormField *)textField {
    // _backspace indicates that the backspace key was typed.
    _backspace = YES;
    
}

- (void)textFieldDidDeleteBackward:(__unused UQUIKFormField *)textField originalText:(__unused NSString *)originalText {
    // To be implemented by subclasses
}

- (void)textField:(__unused UQUIKFormField *)textField willInsertText:(__unused NSString *)text {
    _backspace = NO;
}

- (void)textField:(__unused UQUIKFormField *)textField didInsertText:(__unused NSString *)text {
    // To be implemented by subclasses
}
